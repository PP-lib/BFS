...
import numpy as np
import yaml
import backtest_cryptowatch as backtest_module
yaml_file = "backtest_cryptowatch.yaml"
csv_file = "backtest_cryptowatch_result.csv"
plot_graph = True
...
if __name__ == '__main__':
    ...
    cw = backtest_module.cryptowatch.CryptoWatch()
    df = cw.getCandle(config['backtest_resolution']*60,
                      market, numofcandle=config['backtest_candles'], fill=True)
    print("{} candles".format(len(df)))
    print("First Candle : {}".format(df.index[1]))
    ...
    # 組み合わせテストを行う場合
        ...
        print("Finish all test : {:.3f}sec".format(time.time() - starttime))
