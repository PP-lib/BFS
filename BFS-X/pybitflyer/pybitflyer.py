...
class API(object):
    ...
    def __init__(self, api_key=None, api_secret=None, timeout=None):
        ...
        # API Limit
        self.LimitPeriod = 300
        self.LimitRemaining = 500
        self.LimitReset = time.time()
        self.OrderLimitPeriod = 300
        self.OrderLimitRemaining = 300
        self.OrderLimitReset = time.time()
    ...
    def request(self, endpoint, method="GET", params=None):
        ...
        try:
            ...
            raise e
        try:
            self.LimitPeriod = int(response.headers['X-RateLimit-Period'])
            self.LimitRemaining = int(response.headers['X-RateLimit-Remaining'])
            self.LimitReset = int(response.headers['X-RateLimit-Reset'])
            self.OrderLimitPeriod = int(response.headers['X-OrderRequest-RateLimit-Period'])
            self.OrderLimitRemaining = int(response.headers['X-OrderRequest-RateLimit-Remaining'])
            self.OrderLimitReset = int(response.headers['X-OrderRequest-RateLimit-Reset'])
        except:
            pass
