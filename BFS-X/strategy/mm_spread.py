...
class MyStrategy(Strategy):
    ...
    def realtime_logic(self):
        ...
        if self.current_pos<=self._strategy_config['max_lot'] and self._order_side == 'BUY' :
            self._limit_buy( price=self._order_price, size=self._strategy_config['lotsize'] )
        if self.current_pos>=-self._strategy_config['max_lot'] and self._order_side == 'SELL' :
            self._limit_sell( price=self._order_price, size=self._strategy_config['lotsize'] )
    ...
    def loss_cut_check(self):
        # botの稼働状況表示のみ
        if not self.is_backtesting :
            self._logger.info( '                      LTP:{:.0f}   Profit:{:>+8.0f}({:+4.0f}) Position:{:>7.3f} API:{:>3} API2:{:>3} Delay:{:>4.0f}ms({:>4.0f}ms) {}'.format(
                    self.ltp, self.current_profit, self.current_profit_unreal, self.current_pos, self.api_count, self.api_count2,
